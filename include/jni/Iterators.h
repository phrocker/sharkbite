...
public:
	AccumuloIterator(JNIEnv *env, jobject itr) :
		...
		getTopKeyMethod = env->GetMethodID(iterClass, "getTopKey",
				"()Lorg/apache/accumulo/core/data/Key;");

		///setTopValueMethod = env->GetMethodID(iterClass, "setTopValue", "(Lorg/apache/accumulo/core/data/Value;)V");
		getTopValueMethod = env->GetMethodID(iterClass, "getTopValue",
				"()Lorg/apache/accumulo/core/data/Value;");
		seekMethod = env->GetMethodID(iterClass, "seek",
				"(Lorg/apache/accumulo/core/data/Range;)V");
	}
	...
	bool setTopKey(JNIEnv *env, jobject itr,
			const std::shared_ptr<cclient::data::Key> &passedKey = nullptr) {
		auto clz = env->GetObjectClass(itr);
		auto setTopKeyMethod = env->GetMethodID(clz, "setTopKey",
				"(Lorg/apache/accumulo/core/data/Key;)V");
		auto keyToSet = passedKey == nullptr ? key : passedKey;
		if (!keyToSet)
			return false;
		//return key;
		AccumuloKey keyTransfer(keyToSet);
		env->CallVoidMethod(itr, setTopKeyMethod,
				keyTransfer.getAccumuloKey(env));
		if (env->ExceptionCheck()) {
	...
	bool setTopValue(JNIEnv *env, jobject itr,
			const std::shared_ptr<cclient::data::Value> &passedValue = nullptr) {
		auto clz = env->GetObjectClass(itr);
		auto setTopValueMethod = env->GetMethodID(clz, "setTopValue",
				"(Lorg/apache/accumulo/core/data/Value;)V");
		auto valueToSet = passedValue == nullptr ? value : passedValue;
		if (!valueToSet)
			return false;

		AccumuloValue valueTransfer(valueToSet);
		env->CallVoidMethod(itr, setTopValueMethod,
				valueTransfer.getAccumuloValue(env));
		if (env->ExceptionCheck()) {
			return false;
		}
		return true;
...
public:
	...
	virtual void callSeek(
			const std::shared_ptr<cclient::data::Range> &range) = 0;
