...
class ScannerHeuristic : public Heuristic<interconnect::ThriftTransporter> {
 ...
 public:
  ...
  void disableRpcCalls() { disableRpc = true; }
  ...
  void haltOnException(
      ...
      const std::shared_ptr<interconnect::ServerInterconnect> &server,
      interconnect::Scan *scan, std::exception_ptr e) {
    // close the scanners
    running = false;
    scanResource->src->getResultSet()->setException(e);
  }

  virtual uint16_t scan(Source<cclient::data::KeyValue,
                               ResultBlock<cclient::data::KeyValue>> *source) {
    acquireLock();
 ...
 protected:
  virtual std::shared_ptr<logging::Logger> getLogger() { return logger; }
  ...
  static void closeScan(Source<cclient::data::KeyValue,
                               ResultBlock<cclient::data::KeyValue>> *source) {
  ...
  void addFailedScan(
    ...
    if (NULL != scan) lastKey = scan->getTopKey();
    ...
    for (auto range : *ranges) {
      if (NULL != lastKey && (range->getStopKey()) <= lastKey) {
        ...
        auto newRange = std::make_shared<cclient::data::Range>(
            lastKey, false, range->getStopKey(), range->getStopKeyInclusive());
    ...
    logging::LOG_TRACE(logger)
        << "Adding failed scan " << locatedTablets.size();
    for (auto newRangeDef : locatedTablets) {
      auto directConnect = std::make_shared<interconnect::ServerInterconnect>(
          newRangeDef, scanResource->src->getInstance()->getConfiguration());

      addUniqueConnection(directConnect);
    }
  }

  static void *scanRoutine(
      ScanPair<interconnect::ThriftTransporter> *scanResource) {
    ...
    do {
      ...
      if (NULL != conn) {
        try {
          scan = conn->scan(scanResource->runningFlag, source->getColumns(),
                            source->getIters());
          ...
          ((ScannerHeuristic *)scanResource->heuristic)
          ...
          logging::LOG_TRACE(
              ((ScannerHeuristic *)scanResource->heuristic)->getLogger())
              << "Not serving " << te.what();
          ((ScannerHeuristic *)scanResource->heuristic)
              ->addFailedScan(scanResource, conn, scan);
          ...
          logging::LOG_TRACE(
              ((ScannerHeuristic *)scanResource->heuristic)->getLogger())
              << "Client Exception " << te.what();
          closeScan(source);

          ((ScannerHeuristic *)scanResource->heuristic)
              ->haltOnException(scanResource, conn, scan,
  ...
  void addUniqueConnection(
      const std::shared_ptr<interconnect::ServerInterconnect> &rangeDef) {
    std::shared_ptr<
        interconnect::ClientInterface<interconnect::ThriftTransporter>>
        nextService = NULL;
    ...
    while (!acquireLock()) {
      if (!running) {
        logging::LOG_TRACE(logger)
            << "addUniqueConnection Called, but not running, returning";
        return;
      }
    }
    std::lock_guard<std::timed_mutex> lock(serverLock, std::adopt_lock);
    ...
    logging::LOG_TRACE(logger)
        << " addUniqueConnection Called, servers.size is "
        << std::to_string(servers.size());
    for (const auto &rd : servers) {
      logging::LOG_TRACE(logger) << "Testing range defs";
      if (*rangeDef->getRangesDefinition().get() ==
          *std::dynamic_pointer_cast<interconnect::ServerInterconnect>(rd)
               ->getRangesDefinition()
               .get()) {
        logging::LOG_TRACE(logger)
            << "Range def already in server set. returning";
        return;
      }
    }
    servers.push_back(rangeDef);
    logging::LOG_TRACE(logger) << "addUniqueConnection Called, servers.size is "
                               << std::to_string(servers.size());
  }
  ...
  virtual std::shared_ptr<interconnect::ServerInterconnect> next() {
    ...
    while (!acquireLock()) {
      if (!running) {
        logging::LOG_TRACE(logger)
            << "Next Called, but not running, returning nullptr";
        return nullptr;
      }
    }
    std::lock_guard<std::timed_mutex> lock(serverLock, std::adopt_lock);
    ...
    logging::LOG_TRACE(logger)
        << "Next Called, servers.size is " << std::to_string(servers.size());
    if (!servers.empty()) {
    ...
    std::shared_ptr<interconnect::ServerInterconnect> connector =
        std::dynamic_pointer_cast<interconnect::ServerInterconnect>(
            nextService);
