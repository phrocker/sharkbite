...
/**
 Scanner scans uses the interconnect to retrieve Keys and Values from the data
 source.
 **/
class Scanner : public scanners::Source<cclient::data::KeyValue,
 ...
 protected:
  ...
  void addServerDefinition(
      interconnect::ClientInterface<interconnect::ThriftTransporter> *ifc) {
  ...
  std::vector<cclient::data::IterInfo> getTableIterators(
      std::string iterName = "");
...
using BatchScanner = scanners::Source<cclient::data::KeyValue,
                                      ResultBlock<cclient::data::KeyValue>>;
}  // namespace scanners
#endif /* SCANNER_H_ */
