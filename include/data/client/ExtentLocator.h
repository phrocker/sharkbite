...
class LocatorKey {
 public:
  ...
  LocatorKey(const LocatorKey &other)
      : instance(other.instance), tableName(other.tableName) {}
  /**
   * Constructor
...
class TabletLocator {
 public:
   ...
   * @param binnedmutations binned mutations according to the server hosting the
   *data
   * @param locations servers hosting the data
   * @param failures failed binned locations
   **/
  virtual void binMutations(
      cclient::data::security::AuthInfo *credentials,
   ...
   **/
  virtual void invalidateCache(cclient::data::KeyExtent failedExtent) = 0;
  ...
  virtual void invalidateCache(
      std::vector<cclient::data::KeyExtent> keySet) = 0;
};
