...
/*
  template<typename ONE,typename TWO, typename ...A>
  void testVersion(ONE thiss(A...), TWO other(A...))
  {
	  if (accumuloVersion == ACCUMULO_UNKNOWN){
		  std::function<ONE(A...)>(thiss);
	  }
	  else{
...
*/
 ...
 public:
  ...
  AccumuloMasterFacade(const std::string &host,std::function<void()> fx, std::function<std::shared_ptr<apache::thrift::transport::TTransport>()> tfx)
