...
class AccumuloMasterFacadeV1 : public AccumuloMasterFacade {
 ...
 public:
    ...
    if (wait) {
      while (true) {
        ...
        try {
          ...
          org::apache::accumulo::core::master::thrift::MasterClientServiceClient waitClient(protocolPtr);
  ...
  bool createTable(cclient::data::security::AuthInfo *auth, const std::string &table) override {
  ...
  bool compactFallBack(cclient::data::security::AuthInfo *auth, const std::string &table, const std::string &startrow, const std::string &endrow, bool wait) override {
    return v1_compactFallBack(auth, table, startrow, endrow, wait);
  }

  bool compact(cclient::data::security::AuthInfo *auth, const std::string &table, const std::string &startrow, const std::string &endrow, bool wait) override {
    return v1_compact(auth, table, startrow, endrow, wait);
  }

  bool flush(cclient::data::security::AuthInfo *auth, const std::string &table, const std::string &startrow, const std::string &endrow, bool wait) override {
    return v1_flush(auth, table, startrow, endrow, wait);
  }

  bool removeTable(cclient::data::security::AuthInfo *auth, const std::string &table) override {
    return v1_removeTable(auth, table);
  }

  void removeTableProperty(cclient::data::security::AuthInfo *auth, const std::string &table, const std::string &property) override {
    v1_removeTableProperty(auth, table, property);
  }

  void setTableProperty(cclient::data::security::AuthInfo *auth, const std::string &table, const std::string &property, const std::string &value) override {
    v1_setTableProperty(auth, table, property, value);
  }

  /**namespace operations**/

  bool createNamespace(cclient::data::security::AuthInfo *auth, std::string name) override {
    return v1_createNamespace(auth, name);
  }

  bool deletenamespace(cclient::data::security::AuthInfo *auth, std::string name) override {
    return v1_deletenamespace(auth, name);
  }

  bool renamenamespace(cclient::data::security::AuthInfo *auth, std::string oldName, std::string newName) override {
    return v1_renamenamespace(auth, oldName, newName);
  }

  void removeNamespaceProperty(cclient::data::security::AuthInfo *auth, std::string nameSpaceName, const std::string &property) override {
    v1_removeNamespaceProperty(auth, nameSpaceName, property);
  }

  void setNamespaceProperty(cclient::data::security::AuthInfo *auth, std::string nameSpaceName, const std::string &property, const std::string &value) override {
