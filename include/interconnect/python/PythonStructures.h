...
public:
    explicit PythonTableOperations(const std::shared_ptr<AccumuloTableOperations> &tops) : table_opts(tops){
    }
    ...
    bool create(bool recreate = false) {
        return table_opts->create(recreate);
    }
   ...
   * @return new scanner
   **/
  auto createScanner(cclient::data::security::Authorizations *auths, uint16_t threads) {
      return table_opts->createSharedScanner(auths,threads);
  }

  auto createWriter(cclient::data::security::Authorizations *auths, uint16_t threads) {
...
public:
    explicit PythonNamespaceOperations(const std::shared_ptr<NamespaceOperations> &tops) : namespace_ops(tops){
    ...
    bool remove(std::string name = ""){
        return namespace_ops->remove(name);
    }
    ...
    bool exists(std::string name = ""){
        return namespace_ops->exists(name);
    }
    ...
    void create(std::string name = ""){
        return namespace_ops->create(name);
    }
     ...
     * @param value property value
     */
    void setProperty(std::string property, std::string value, std::string nm = ""){
...
public:
    explicit PythonSecurityOperations(const std::shared_ptr<SecurityOperations> &tops) : security_ops(tops){
  ...
  int8_t dropUser(std::string user){
      return security_ops->dropUser(user);
  }
