 ...
 * Design: Isa ThriftTransporter and FateInterface, therefore it allows for fate
 *operations
 **/
class AccumuloCoordinatorTransporter : public ThriftTransporter,
                                       public FateInterface {
 private:
 ...
 protected:
  ...
  void switchInterconnect(int version = -1) override {
    if (version == -1) {
      version = cclient::data::InstanceVersion::getVersion(
          getConnection()->toString());
    }
    switch (version) {
      case -1:
      case 1:
        coordinator = std::make_unique<AccumuloCoordinatorFacadeV1>(
            getConnection()->getHost(), [&]() { recreateCoordinatorClient(); },
        ...
        break;
      case 2:
        coordinator = std::make_unique<AccumuloCoordinatorFacadeV2>(
            getConnection()->getHost(), [&]() { recreateCoordinatorClient(); },
            [&]() -> std::shared_ptr<apache::thrift::transport::TTransport> {
              return createTransporter();
            });
 ...
 public:
  ...
  explicit AccumuloCoordinatorTransporter(
      ...
      : interconnect::ThriftTransporter(conn, false),
        ...
        logger(logging::LoggerFactory<
               AccumuloCoordinatorTransporter>::getLogger()) {
    switchInterconnect();
    createCoordinatorClient();
    createClientService(false);
  }
  ...
  bool createTable(cclient::data::security::AuthInfo *auth,
    ...
    return callApiBool(
        [&]() -> bool { return coordinator->createTable(auth, table); });
  }
  ...
  bool importDirectory(cclient::data::security::AuthInfo *auth,
    ...
    return callApiBool([&]() -> bool {
      return coordinator->importDirectory(auth, table, dir, failure_dir,
                                          setTime);
    });
  }
  ...
  cclient::data::AccumuloInfo getStatistics(
    ...
    return callApiWith<cclient::data::AccumuloInfo>(
        [&]() -> cclient::data::AccumuloInfo {
  ...
  bool removeTable(cclient::data::security::AuthInfo *auth,
                   const std::string &table) {
    return callApiBool(
        [&]() -> bool { return coordinator->removeTable(auth, table); });
  }
  ...
  bool createNamespace(cclient::data::security::AuthInfo *auth,
                       std::string name) {
    return callApiBool(
        [&]() -> bool { return coordinator->createNamespace(auth, name); });
  }

  bool deletenamespace(cclient::data::security::AuthInfo *auth,
                       std::string name) {
    return callApiBool(
        [&]() -> bool { return coordinator->deletenamespace(auth, name); });
  }
  ...
  void removeNamespaceProperty(cclient::data::security::AuthInfo *auth,
                               ...
                               const std::string &property) {
    callAPi([&]() {
      return coordinator->removeNamespaceProperty(auth, nameSpaceName,
                                                  property);
    });
  }
  ...
  void setNamespaceProperty(cclient::data::security::AuthInfo *auth,
                            ...
                            const std::string &value) {
    callAPi([&]() {
      return coordinator->setNamespaceProperty(auth, nameSpaceName, property,
                                               value);
    });
  }
