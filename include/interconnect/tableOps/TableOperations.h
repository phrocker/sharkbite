...
public:
    ...
    /**
      ...
      * @return new scanner
      **/
    virtual std::unique_ptr<scanners::Source<K, V>> createScanner(cclient::data::security::Authorizations *auths,
                                        uint16_t threads) = 0;
    ...
    /**				
      ...
      */
    virtual std::unique_ptr<writer::Sink<K>> createWriter(cclient::data::security::Authorizations *auths,
                                  uint16_t threads) = 0;
...
void TableOperations<K, V>::createTable(std::string table) {
  ...
  **/
template<typename K, typename V>
TableOperations<K, V>::TableOperations(cclient::data::security::AuthInfo *creds, cclient::data::Instance *instance,
                                       std::string table) :
