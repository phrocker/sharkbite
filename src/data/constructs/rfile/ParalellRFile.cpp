...
ParallelRFile::ParallelRFile(std::function<std::vector<cclient::data::streams::FileStream>()> streamfn, int versions, bool withDeletes, bool propogate)
    : streamfunction(streamfn), versions(versions),withDeletes(withDeletes),propogate(propogate),
...
void ParallelRFile::relocate(cclient::data::streams::StreamRelocation *location) {
    close();
    ...
    if (!location->getRange()->getInfiniteStartKey() && !location->getRange()->getInfiniteStopKey()){
        number = 1;
    }
    ...
    std::vector<cclient::data::streams::FileStream> filestreams = streamfunction();
    {
        GroupedReaders rdrs;
        for(auto &filestream : filestreams){
            auto stream = std::make_shared<cclient::data::SequentialRFile>(std::move(filestream.stream),(long)filestream.filesize);
            std::vector<std::shared_ptr<cclient::data::Key>> ks = stream->getBlocks(location);
            key_vectors.assign(ks.begin(),ks.end());
            rdrs.readers.push_back(stream);
        }
        readers.emplace_back(rdrs);
    }
    // sort and unique. while this could take a while for very large RFiles, this should be lower than the aggregate
    ...
    // after we've sorted and ensured we have a defined set of ranges we need to re-open RFiles to match
    // the ranges.
    ...
    for(const auto &reader_set : readers){
        std::shared_ptr<cclient::data::streams::KeyValueIterator> heapItr = {};
         if (versions == 0) {
            if (!withDeletes) {
            heapItr = std::make_shared<cclient::data::MultiIterator>(reader_set.readers);
            } else {
            heapItr = std::make_shared<cclient::data::DeletingMultiIterator>(
                reader_set.readers, propogate);
            }
        } else {
            heapItr = std::make_shared<cclient::data::VersioningIterator>(reader_set.readers);
        }
    ...
    if (location->getRange()->getInfiniteStartKey() && location->getRange()->getInfiniteStopKey()){
        auto binnedRanges = binRanges(key_vectors.begin(),key_vectors.end(),number-1);

        /**
         * TODO: Currently this only supports (-inf,+inf). This should be extended
         **/
        keys.emplace_back(std::make_shared<cclient::data::Range>(*binnedRanges.at(0).first,false));
        for(const auto &pair : binnedRanges){
            // create a range 
            if (*pair.second != nullptr){
                keys.emplace_back(std::make_shared<cclient::data::Range>(*pair.first,true,*pair.second,false));
            }
            else{
                keys.emplace_back(std::make_shared<cclient::data::Range>(true,*pair.first));
            }
        }
    }
    else{
        keys.emplace_back(std::make_shared<cclient::data::Range>(location->getRange()->getStartKey(),location->getRange()->getStartKeyInclusive(),location->getRange()->getStopKey(),location->getRange()->getStopKeyInclusive()));
    }
    assert( keys.size() == thread_readers.size() == number);

    for (uint32_t i=0 ; i < number; i++){
        thread_readers.at(i)->range = std::make_shared<cclient::data::streams::DelegatedStreamSeekable>(keys.at(i),location);
    }

    readersResult.maxThread = number;
    readersResult.threadCacheCount = location->getKeysCachedPerThread();

    // start a delegator that delegate the ranges to the  worker threads, keeping in mind that data must come in order from those worker threads
    startDelegator();
    // 0 , 1 , 2 , ... n

    // pull data from 0, if empty go to one, allowing zero to take next range, same with one, two , etc until n, at which point we'll return
    // to 0 and start all over. We'll use a circular queue of ranges. 
    hasNext();
    next();
  }
  ...
  std::pair<std::shared_ptr<cclient::data::Key>, std::shared_ptr<cclient::data::Value>> ParallelRFile::operator*() {
      //readersResult.resultSet.try_dequeue(current_kv);
  return std::make_pair(current_kv->getKey(),
                        current_kv->getValue());
}
    ...
    for(uint16_t i=0; i < readersResult.maxThread; i++){
        ...
        std::function<utils::TaskRescheduleInfo()> f_ex = [&] {
            ...
            while (readersRunning && my_reader->heapIter->hasNext()) {
                while (readersRunning && !my_reader->resultSet.try_enqueue( my_reader->heapIter->getTop()) ){
                    std::this_thread::sleep_for(std::chrono::milliseconds(1));
                }
                while (readersRunning && (readersResult.currentThread != index && my_reader->resultSet.size_approx() > readersResult.threadCacheCount)){
                    std::this_thread::sleep_for(std::chrono::milliseconds(2));
                }
        ...
        utils::Worker<utils::TaskRescheduleInfo> functor(f_ex,std::to_string(i), std::move(monitor));
     ...
     std::function<utils::TaskRescheduleInfo()> d_ex = [&] {
        ...
        // only need to run this once.
        auto monitor = std::make_unique<utils::ComplexMonitor>();
        utils::Worker<utils::TaskRescheduleInfo> functor(d_ex,"KeyDelegator", std::move(monitor));
