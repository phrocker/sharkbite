...
SerializedIndex::SerializedIndex(const std::vector<int> &offsetList,
                                 uint8_t *datums, uint32_t dataLength,
                                 bool newFormat)
    : newFormat(newFormat),
...
SerializedIndex::SerializedIndex(const std::shared_ptr<BaseMetaBlock> &source,
                                 const std::shared_ptr<cclient::data::Key> &key,
                                 const std::shared_ptr<BaseMetaBlock> &block)
...
std::shared_ptr<IndexEntry> SerializedIndex::get(uint64_t index) {
  ...
  if (index >= offsets.size()) return nullptr;

  len = dataLength - offsets.at(index);

  std::shared_ptr<IndexEntry> returnKey =
      std::make_shared<IndexEntry>(newFormat);
...
SerializedIndex &SerializedIndex::operator++() {
...
SerializedIndex &SerializedIndex::operator++(int t) {
  // if we don't have offsets, then use ptr
  // if we do have offsets, then we should increment
  if (offsets.size() == 0) {
    if (ptr->currentPosition + 1 < ptr->offsets.size()) {
...
bool SerializedIndex::hasNext() {
  if ((nullptr == blockParty && offsets.empty())) return false;
...
uint32_t SerializedIndex::getPreviousIndex() {
  if (NULL == blockParty) return 0;
  int offset = blockParty->getOffset();
  int prev = 0;
  if (NULL != ptr) prev = ptr->getPreviousIndex();
  return offset + prev;
}
...
std::shared_ptr<IndexEntry> SerializedIndex::previous() {
  ...
  if (ptr == shared_from_this()) return 0;
  return ptr->previous();
}
