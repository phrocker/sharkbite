...
std::shared_ptr<cclient::data::SequentialRFile> RFileOperations::openForWrite(const std::string &rfile, uint32_t compress_blocK_size) {
  ...
  auto compressor = std::make_unique<cclient::data::compression::ZLibCompressor>(compress_blocK_size);
  ...
  if (rfiles.size() > 1){
    ...
    for(auto &future : futures){
        ...
        std::string path = rfiles.at(0);
          ...
          iters.emplace_back(std::make_shared<cclient::data::SequentialRFile>(std::move(endstream), size));
  }
  if (versions==0)
