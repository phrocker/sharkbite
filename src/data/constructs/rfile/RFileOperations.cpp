...
std::shared_ptr<cclient::data::streams::KeyValueIterator>
RFileOperations::openParallelRFiles(
    const std::vector<std::string> &rfiles,
    const std::shared_ptr<cclient::data::KeyPredicate> &predicate, int versions,
    bool withDeletes, bool propogate, uint64_t maxtimestamp) {
  std::function<std::vector<cclient::data::streams::FileStream>()> fn =
      [rfiles, predicate, versions, withDeletes, propogate,
       maxtimestamp](void) -> std::vector<cclient::data::streams::FileStream> {
    ...
    for (const auto &path : rfiles) {
      ...
      std::unique_ptr<cclient::data::streams::InputStream> endstream =
          std::make_unique<cclient::data::streams::ReadAheadInputStream>(
              std::move(stream), 128 * 1024, 1024 * 1024, size);
      streams.emplace_back(
          cclient::data::streams::FileStream(std::move(endstream), size));
    }
    return streams;
  };
  return std::make_shared<ParallelRFile>(std::move(fn), versions, withDeletes,
                                         propogate);
}
