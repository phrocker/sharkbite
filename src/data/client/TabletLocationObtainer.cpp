...
std::map<std::shared_ptr<cclient::data::Key>, std::shared_ptr<cclient::data::Value>, pointer_comparator<std::shared_ptr<cclient::data::Key>>> TabletLocationObtainer::decodeResults(
  ...
  for (std::vector<std::shared_ptr<cclient::data::KeyValue> >::iterator it = results->begin(); it != results->end(); it++) {
    ...
    cclient::data::streams::EndianInputStream *byteStream = new cclient::data::streams::EndianInputStream((char*) pair.first, pair.second);
...
inline std::pair<uint32_t, uint8_t*> TabletLocationObtainer::readByteArray(cclient::data::streams::InputStream *stream) {
