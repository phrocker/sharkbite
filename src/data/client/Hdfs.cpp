...
int HdfsLink::chown(const std::string &dir, const std::string &user, const std::string &group) {
  return hdfsChown(reference, dir.c_str(), user.c_str(), group.c_str());
}

std::shared_ptr<cclient::data::streams::ByteOutputStream> HdfsLink::write(const std::string &path){
  return std::make_shared<cclient::data::streams::HdfsOutputStream>(shared_from_this(),path);
}

std::shared_ptr<cclient::data::streams::InputStream> HdfsLink::read(const std::string &path){
  return std::make_shared<cclient::data::streams::HdfsInputStream>(shared_from_this(),path);
}
