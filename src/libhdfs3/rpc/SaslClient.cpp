...
void SaslClient::initKerberos(const RpcSaslProto_SaslAuth& auth,
  ...
  if ((rc = gsasl_client_start(ctx, auth.mechanism().c_str(), &session)) !=
      GSASL_OK) {
...
std::string SaslClient::evaluateChallenge(const std::string& challenge) {
  ...
  if (rc == GSASL_NEEDS_MORE || rc == GSASL_OK) {
    ...
    THROW(AccessControlException, "Failed to evaluate challenge: %s",
          gsasl_strerror(rc));
  }
...
bool SaslClient::isComplete() { return complete; }
