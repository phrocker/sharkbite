...
RpcChannelImpl::RpcChannelImpl(const RpcChannelKey &k, RpcClient &c)
  ...
  in = shared_ptr<BufferedSocketReader>(
      new BufferedSocketReaderImpl(*static_cast<TcpSocketImpl *>(sock.get())));
  ...
  do {
    ...
    switch (response.state()) {
      case RpcSaslProto_SaslState_NEGOTIATE: {
        const RpcSaslProto_SaslAuth *auth = createSaslClient(&response.auths());
...
void RpcChannelImpl::invoke(const RpcCall &call) {
  ...
  try {
    ...
    do {
      ...
      remote =
          RpcRemoteCallPtr(new RpcRemoteCall(call, id, client.getClientId()));
      ...
      if (lastError) {
        ...
        if (!retry && call.isIdempotent()) {
          ...
          LOG(INFO, "Retry idempotent RPC call \"%s\" on server \"%s:%s\"",
              call.getName(), key.getServer().getHost().c_str(),
              key.getServer().getPort().c_str());
        } else {
          rethrow_exception(lastError);
    ...
    if (lastError == exception_ptr()) {
      try {
        THROW(Hdfs::HdfsRpcException,
              "Failed to invoke RPC call \"%s\", RPC channel to \"%s:%s\" is "
              "to be closed since RpcClient is closing",
              call.getName(), key.getServer().getHost().c_str(),
              key.getServer().getPort().c_str());
      } catch (...) {
        lastError = current_exception();
...
void RpcChannelImpl::checkOneResponse() {
  ...
  while (client.isRunning()) {
    if (getResponse()) {
      ...
      if (ping > 0 &&
          ToMilliSeconds(lastActivity, steady_clock::now()) >= ping) {
        lock_guard<mutex> lock(writeMut);
        sendPing();
      }
    }
...
void RpcChannelImpl::sendPing() {
  ...
  if (available) {
    LOG(INFO,
        "RPC channel to \"%s:%s\" got no response or idle for %d milliseconds, "
        "sending ping.",
        key.getServer().getHost().c_str(), key.getServer().getPort().c_str(),
        key.getConf().getPingTimeout());
...
RpcRemoteCallPtr RpcChannelImpl::getPendingCall(int32_t id) {
  ...
  if (it == pendingCalls.end()) {
    THROW(HdfsRpcException,
          "RPC channel to \"%s:%s\" got protocol mismatch: RPC channel cannot "
          "find pending call: id = %d.",
          key.getServer().getHost().c_str(), key.getServer().getPort().c_str(),
          static_cast<int>(id));
  }
...
static exception_ptr HandlerRpcResponseException(exception_ptr e) {
  ...
  try {
    ...
    UnWrapper<NameNodeStandbyException, RpcNoSuchMethodException,
              ...
              SafeModeException, SaslException>
        unwrapper(e);
...
void RpcChannelImpl::readOneResponse(bool writeLock) {
  ...
  // use ParseFromCodedStream instead of ParseFromArray, so it can consume the
  // above CodedInputStream
  //
  // if just use ParseFromArray, we have no chance to set TotalBytesLimit (64MB
  // default)
  ...
  if (RpcResponseHeaderProto_RpcStatusProto_SUCCESS == status) {
    ...
    if (RpcResponseHeaderProto_RpcStatusProto_ERROR == status) {
      ...
      if (errClass.empty()) {
        THROW(HdfsRpcException, "%s: %s", errClass.c_str(), errMessage.c_str());
      }
