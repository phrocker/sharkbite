...
const char* SeverityName[] = {"FATAL",  "ERROR",  "WARNING", "INFO",
...
Logger::Logger() : fd(STDERR_FILENO), severity(DEFAULT_LOG_LEVEL) {}

Logger::~Logger() {}

void Logger::setOutputFd(int f) { fd = f; }

void Logger::setLogSeverity(LogSeverity l) { severity = l; }
  ...
  try {
    ...
    size =
        snprintf(&buffer[0], buffer.size(),
                 ...
                 static_cast<long>(tval.tv_usec), ProcessId, SeverityName[s]);
    va_start(ap, fmt);
