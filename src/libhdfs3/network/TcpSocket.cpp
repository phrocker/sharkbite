...
TcpSocketImpl::TcpSocketImpl() : sock(-1), lingerTimeout(-1) {}
...
void TcpSocketImpl::connect(struct addrinfo *paddr, const char *host,
   ...
   * It is OK to set a very large value here since the user has its own timeout
   * mechanism.
   */
  ...
  try {
    ...
    if (rc < 0) {
      if (EINPROGRESS != errno && EWOULDBLOCK != errno) {
        if (ETIMEDOUT == errno) {
          THROW(HdfsTimeoutException, "Connect to \"%s:%s\" timeout", host,
                port);
        } else {
