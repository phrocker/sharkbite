...
KerberosName::KerberosName(const std::string& principal) { parse(principal); }
...
void KerberosName::parse(const std::string& principal) {
  ...
  try {
    ...
    rc = regexec(&comp, principal.c_str(), sizeof(pmatch) / sizeof(pmatch[1]),
                 pmatch, 0);
    ...
    if (rc == REG_NOMATCH) {
      if (principal.find('@') != principal.npos) {
        THROW(HdfsIOException, "KerberosName: Malformed Kerberos name: %s",
              principal.c_str());
      } else {
        name = principal;
      }
...
size_t KerberosName::hash_value() const {
  size_t values[] = {StringHasher(name), StringHasher(host),
                     StringHasher(realm)};
