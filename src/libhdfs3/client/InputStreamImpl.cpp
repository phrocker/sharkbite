...
unordered_set<std::string> BuildLocalAddrSet() {
  ...
  try {
    ...
    if (gethostname(&host[0], host.size())) {
      THROW(HdfsNetworkException, "InputStreamImpl: cannot get hostname: %s",
            GetSystemErrorInfo(errno));
    }
...
int64_t InputStreamImpl::readBlockLength(const LocatedBlock &b) {
  ...
  for (size_t i = 0; i < nodes.size(); ++i) {
    try {
      ...
      std::string buffer;
      LOG(LOG_ERROR,
          "InputStreamImpl: failed to get block visible length for Block: %s "
          "file %s from Datanode: %s\n%s",
          b.toString().c_str(), path.c_str(), nodes[i].formatAddress().c_str(),
          GetExceptionDetail(e, buffer));
      LOG(INFO,
          "InputStreamImpl: retry get block visible length for Block: %s file "
          "%s from other datanode",
...
void InputStreamImpl::updateBlockInfos() {
  ...
  for (int i = 0; i < retry; ++i) {
          ...
          if (lastBlockBeingWrittenLength == -1) {
            if (i + 1 >= retry) {
              THROW(HdfsIOException,
                    ...
                    "Block: %s from all Datanode.",
                    last->toString().c_str());
            } else {
              LOG(LOG_ERROR,
                  "InputStreamImpl: failed to get block visible length for "
                  "Block: %s file %s from all Datanode.",
    ...
    LOG(INFO,
        "InputStreamImpl: retry to get block information for file: %s, already "
        "tried %d time(s).",
        path.c_str(), i + 1);
  }
}
...
void InputStreamImpl::setupBlockReader(bool temporaryDisableLocalRead) {
  ...
  while (true) {
    if (!choseBestNode()) {
      ...
      THROW(HdfsIOException,
            "InputStreamImpl: all nodes have been tried and no valid replica "
            "can be read for Block: %s.",
            curBlock->toString().c_str());
    }
    ...
    try {
      ...
      /**
       * check the domain socket path too. if it is not set let's avoid an
       * upcoming exception and access the data node.
       */
      if (!temporaryDisableLocalRead && !lastReadFromLocal &&
          !readFromUnderConstructedBlock && localRead && isLocalNode() &&
          !conf->getDomainSocketPath().empty()) {
        ...
        try {
          ...
          assert(info->isValid());
          blockReader = shared_ptr<BlockReader>(new LocalBlockReader(
              info, *curBlock, offset, verify, *conf, localReaderBuffer));
        } catch (...) {
          if (info) {
            info->setValid(false);
          }
        ...
        blockReader = shared_ptr<BlockReader>(new RemoteBlockReader(
            *curBlock, curNode, *peerCache, offset, len, curBlock->getToken(),
      ...
      if (lastReadFromLocal) {
        LOG(LOG_ERROR,
            "cannot setup block reader for Block: %s file %s on Datanode: "
            "%s.\n%s\n"
        ...
        /*
         * do not add node into failedNodes since we will retry the same node
         * but disable local block reading
         */
      } else {
        LOG(LOG_ERROR,
            "cannot setup block reader for Block: %s file %s on Datanode: "
...
void InputStreamImpl::openInternal(shared_ptr<FileSystemInter> fs,
...
#if USE_KRB5
      if (cryptoCodec == NULL) {
        enAuth = shared_ptr<RpcAuth>(new RpcAuth(
            fs->getUserInfo(), RpcAuth::ParseMethod(conf->getKmsMethod())));
        kcp =
            shared_ptr<KmsClientProvider>(new KmsClientProvider(enAuth, conf));
...
int32_t InputStreamImpl::read(char *buf, int32_t size) {
  ...
  try {
    ...
    LOG(DEBUG3,
        "%p read file %s size is %d, offset %" PRId64
        " done %d, next pos %" PRId64,
  ...
  while (true) {
    try {
      ...
      LOG(LOG_ERROR,
          ...
          "after updating block informations.",
          curBlock->toString().c_str(), path.c_str(),
          GetExceptionDetail(e, buffer));
      return -1;
    } catch (const HdfsIOException &e) {
    ...
    try {
       ...
       */
      LOG(LOG_ERROR,
          "InputStreamImpl: failed to read Block: %s file %s from Datanode: "
          "%s, \n%s, "
    ...
    if (!blockReader || dynamic_cast<LocalBlockReader *>(blockReader.get())) {
       ...
       */
      LOG(INFO,
          "IntputStreamImpl: Add invalid datanode %s to failed datanodes and "
          "try another datanode again for file %s.",
...
void InputStreamImpl::seekInternal(int64_t pos) {
  ...
  try {
    ...
    std::string buffer;
    LOG(LOG_ERROR,
        "InputStreamImpl: failed to skip %" PRId64
        " bytes in current block reader for file %s\n%s",
        pos - cursor, path.c_str(), GetExceptionDetail(e, buffer));
    LOG(INFO,
        "InputStreamImpl: retry to seek to position %" PRId64 " for file %s",
        pos, path.c_str());
  } catch (const ChecksumException &e) {
    std::string buffer;
    LOG(LOG_ERROR,
