...
std::string CryptoCodec::calculateIV(const std::string &initIV,
...
std::string CryptoCodec::getDecryptedKeyFromKms() {
  ...
  LOG(DEBUG3, "CryptoCodec : getDecryptedKeyFromKms material is :%s",
      key.c_str());
...
int CryptoCodec::init(CryptoMethod crypto_method, int64_t stream_offset) {
  ...
  is_init = true;
  // Calculate iv and counter in order to init cipher context with cipher
  // method. Default value is 0.
  ...
  LOG(DEBUG3,
      "CryptoCodec init success, length of the decrypted key is : %llu, crypto "
      "method is : %d",
      AlgorithmBlockSize, crypto_method);
  return 1;
}

int CryptoCodec::resetStreamOffset(CryptoMethod crypto_method,
                                   int64_t stream_offset) {
  // Check CryptoCodec init or not.
  if (is_init == false) return -1;
...
std::string CryptoCodec::cipher_wrap(const char *buffer, int64_t size) {
  if (!is_init) THROW(InvalidParameter, "CryptoCodec isn't init");
  ...
  while (remaining > bufSize) {
    ret = EVP_CipherUpdate(cipherCtx, (unsigned char *)&out_buf[offset], &len,
                           (const unsigned char *)in_buf.data() + offset,
                           bufSize);

    if (!ret) {
      std::string err = ERR_lib_error_string(ERR_get_error());
      THROW(HdfsIOException,
            "CryptoCodec : cipher_wrap AES data failed:%s, crypto_method:%d",
            err.c_str(), method);
    }
    offset += len;
    remaining -= len;
