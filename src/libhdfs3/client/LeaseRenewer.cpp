...
int LeaseRenewerImpl::getInterval() const { return interval; }

void LeaseRenewerImpl::setInterval(int interval) { this->interval = interval; }
...
void LeaseRenewerImpl::renewer() {
  ...
  while (!stop) {
    try {
      ...
      LOG(LOG_ERROR, "Lease renewer will exit since unexpected exception: %s",
          e.what());
      break;
    }
  }
