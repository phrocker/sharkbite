...
DataTransferProtocolSender::DataTransferProtocolSender(
    Socket& sock, int writeTimeout, const std::string& datanodeAddr)
...
void DataTransferProtocolSender::writeBlock(
    const ExtendedBlock& blk, const Token& blockToken, const char* clientName,
    const std::vector<DatanodeInfo>& targets, int stage, int pipelineSize,
    int64_t minBytesRcvd, int64_t maxBytesRcvd, int64_t latestGenerationStamp,
...
void DataTransferProtocolSender::transferBlock(
    const ExtendedBlock& blk, const Token& blockToken, const char* clientName,
  ...
  try {
    ...
    NESTED_THROW(HdfsIOException,
                 "DataTransferProtocolSender cannot send transfer request to "
                 "datanode %s.",
                 datanode.c_str());
  }
}
...
void DataTransferProtocolSender::blockChecksum(const ExtendedBlock& blk,
  ...
  try {
    ...
    NESTED_THROW(HdfsIOException,
                 "DataTransferProtocolSender cannot send checksum request to "
                 "datanode %s.",
                 datanode.c_str());
  }
}
