...
shared_ptr<Socket> RemoteBlockReader::getNextPeer(const DatanodeInfo& dn) {
  ...
  try {
    ...
    NESTED_THROW(HdfsIOException, "RemoteBlockReader: Failed to connect to %s",
...
void RemoteBlockReader::checkResponse() {
  ...
  if (respSize <= 0 || respSize > 10 * 1024 * 1024) {
    THROW(HdfsIOException,
          "RemoteBlockReader get a invalid response size: %d, Block: %s, from "
          "Datanode: %s",
  ...
  if (!resp.ParseFromArray(&respBuffer[0], respBuffer.size())) {
    THROW(HdfsIOException,
          "RemoteBlockReader cannot parse BlockOpResponseProto from Datanode "
          "response, "
  ...
  if (resp.status() != Status::DT_PROTO_SUCCESS) {
    ...
    if (resp.status() == Status::DT_PROTO_ERROR_ACCESS_TOKEN) {
      THROW(HdfsInvalidBlockToken,
            "RemoteBlockReader: block's token is invalid. Datanode: %s, Block: "
            "%s",
            datanode.formatAddress().c_str(), binfo.toString().c_str());
    } else {
      THROW(HdfsIOException,
            "RemoteBlockReader: Datanode return an error when sending read "
            "request to Datanode: %s, Block: %s, %s.",
            datanode.formatAddress().c_str(), binfo.toString().c_str(),
  ...
  if (chunkSize < 0) {
    THROW(HdfsIOException,
          "RemoteBlockReader invalid chunk size: %d, expected range[0, %" PRId64
          "], Block: %s, from Datanode: %s",
  ...
  switch (cs.type()) {
    ...
    case ChecksumTypeProto::CHECKSUM_CRC32:
      THROW(HdfsIOException,
            "RemoteBlockReader does not support CRC32 checksum, Block: %s, "
            "from Datanode: %s",
            binfo.toString().c_str(), datanode.formatAddress().c_str());
      break;
    ...
    default:
      THROW(HdfsIOException,
            "RemoteBlockReader cannot recognize checksum type: %d, Block: %s, "
            "from Datanode: %s",
...
shared_ptr<PacketHeader> RemoteBlockReader::readPacketHeader() {
  try {
    ...
    if (lastHeader && lastHeader->isLastPacketInBlock()) {
      THROW(HdfsIOException,
            "RemoteBlockReader: read over block end from Datanode: %s, Block: "
            "%s.",
            datanode.formatAddress().c_str(), binfo.toString().c_str());
    }
    ...
    NESTED_THROW(HdfsIOException,
                 "RemoteBlockReader: failed to read block header for Block: %s "
                 "from Datanode: %s.",
                 binfo.toString().c_str(), datanode.formatAddress().c_str());
  }
}
...
void RemoteBlockReader::readNextPacket() {
  ...
  if (!lastHeader->sanityCheck(lastSeqNo)) {
    THROW(HdfsIOException,
          "RemoteBlockReader: Packet failed on sanity check for block %s from "
          "Datanode %s.",
          binfo.toString().c_str(), datanode.formatAddress().c_str());
  }
  ...
  if (dataSize > 0) {
    ...
    if (lastHeader->getPacketLen() !=
        static_cast<int>(sizeof(int32_t)) + dataSize + checksumLen) {
      THROW(HdfsIOException,
            "Invalid Packet, packetLen is %d, dataSize is %d, checksum size is "
            "%d",
...
bool RemoteBlockReader::readTrailingEmptyPacket() {
  ...
  if (!trailingHeader->isLastPacketInBlock() ||
      trailingHeader->getDataLen() != 0) {
    return false;
  }
  ...
  for (int i = 0; i < chunks; ++i) {
    ...
    if (result != target) {
      THROW(ChecksumException,
            "RemoteBlockReader: checksum not match for Block: %s, on Datanode: "
            "%s",
            binfo.toString().c_str(), datanode.formatAddress().c_str());
    }
  }
...
int32_t RemoteBlockReader::read(char* buf, int32_t len) {
  ...
  if (cursor >= endOffset) {
    THROW(
        HdfsIOException,
        "RemoteBlockReader: read over block end from Datanode: %s, Block: %s.",
        datanode.formatAddress().c_str(), binfo.toString().c_str());
  }
  ...
  try {
    ...
    NESTED_THROW(
        HdfsIOException,
        "RemoteBlockReader: failed to read Block: %s from Datanode: %s.",
        binfo.toString().c_str(), datanode.formatAddress().c_str());
...
void RemoteBlockReader::skip(int64_t len) {
  ...
  try {
    while (todo > 0) {
      if (cursor >= endOffset) {
        THROW(HdfsIOException,
              "RemoteBlockReader: skip over block end from Datanode: %s, "
              "Block: %s.",
              datanode.formatAddress().c_str(), binfo.toString().c_str());
      }
