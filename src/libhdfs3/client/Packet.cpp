...
void Packet::setSyncFlag(bool sync) { syncBlock = sync; }

void Packet::increaseNumChunks() { ++numChunks; }

bool Packet::isFull() { return numChunks >= maxChunks; }

bool Packet::isHeartbeat() { return HEART_BEAT_SEQNO == seqno; }

void Packet::setLastPacketInBlock(bool lastPacket) {
  lastPacketInBlock = lastPacket;
}

int Packet::getDataSize() { return dataPos - dataStart; }
...
const ConstPacketBuffer Packet::getBuffer() {
  ...
  header.writeInBuffer(&buffer[headerStart], PacketHeader::GetPkgHeaderSize());
