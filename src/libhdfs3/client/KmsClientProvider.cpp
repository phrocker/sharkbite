...
std::string KmsClientProvider::base64Encode(const std::string &data) {
  ...
  if (GSASL_OK !=
      (rc = gsasl_base64_to(data.data(), data.size(), &buffer, &len))) {
...
std::string KmsClientProvider::base64Decode(const std::string &data) {
  ...
  if (GSASL_OK !=
      (rc = gsasl_base64_from(data.data(), data.size(), &buffer, &len))) {
    assert(GSASL_MALLOC_ERROR == rc);
    throw std::bad_alloc();
  }
 ...
 */
KmsClientProvider::KmsClientProvider(shared_ptr<RpcAuth> rpcAuth,
                                     shared_ptr<SessionConfig> config)
    : auth(rpcAuth), conf(config) {
 ...
 */
std::string KmsClientProvider::buildKmsUrl(const std::string &url,
  ...
  if (method == AuthMethod::KERBEROS) {
    ...
    if (user.length() == 0) user = auth->getUser().getKrbName();
 ...
 */
void KmsClientProvider::createKey(const std::string &keyName,
                                  const std::string &cipher, const int length,
  ...
  LOG(DEBUG3,
      "KmsClientProvider::createKey : The key name, key cipher, key length, "
      "key material, description are : %s, %s, %d, %s, %s. The kms url is : %s "
 ...
 */
ptree KmsClientProvider::getKeyMetadata(
    const FileEncryptionInfo &encryptionInfo) {
  hc->init();
  url = parseKmsUrl();
  std::string urlSuffix =
      "key/" + hc->escape(encryptionInfo.getKeyName()) + "/_metadata";
  url = buildKmsUrl(url, urlSuffix);
  ...
  LOG(DEBUG3,
      "KmsClientProvider::getKeyMetadata : The kms url is : %s. The response "
      "of kms server is : %s .",
...
void KmsClientProvider::deleteKey(const FileEncryptionInfo &encryptionInfo) {
  ...
  LOG(DEBUG3,
      "KmsClientProvider::deleteKey : The kms url is : %s. The response of kms "
      "server is : %s .",
      url.c_str(), response.c_str());
}
 ...
 */
ptree KmsClientProvider::decryptEncryptedKey(
    const FileEncryptionInfo &encryptionInfo) {
  hc->init();
  /* Prepare HttpClient url. */
  url = parseKmsUrl();
  std::string urlSuffix = "keyversion/" +
                          hc->escape(encryptionInfo.getEzKeyVersionName()) +
                          "/_eek?eek_op=decrypt";
  url = buildKmsUrl(url, urlSuffix);
  ...
  LOG(DEBUG3,
      "KmsClientProvider::decryptEncryptedKey : The kms url is : %s . The kms "
      "body is : %s. The response of kms server is : %s .",
