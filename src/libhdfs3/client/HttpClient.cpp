...
size_t HttpClient::CurlWriteMemoryCallback(void *contents, size_t size,
  ...
  LOG(DEBUG3, "HttpClient : Http response is : %s",
...
void HttpClient::init() {
  ...
  CURL_SETOPT_ERROR2(curl, CURLOPT_WRITEFUNCTION,
                     HttpClient::CurlWriteMemoryCallback,
                     "Cannot initialize body reader in HttpClient: %s: %s");

  CURL_SETOPT_ERROR2(
      curl, CURLOPT_WRITEDATA, (void *)&response,
...
void HttpClient::setRequestRetryTimes(int request_retry_times) {
  if (request_retry_times < 0) {
    THROW(InvalidParameter,
          "HttpClient : Invalid value for request_retry_times.");
  }
 ...
 */
void HttpClient::setBody(const std::string &body) { this->body = body; }
...
std::string HttpClient::httpCommon(httpMethod method) {
  ...
  LOG(DEBUG3,
      "HttpClient : The http method is %d. The http url is %s. The http "
      "response is %s.",
 ...
 * Http GET method.
 */
std::string HttpClient::get() { return httpCommon(HTTP_GET); }

/**
 * Http POST method.
 */
std::string HttpClient::post() { return httpCommon(HTTP_POST); }

/**
 * Http DELETE method.
 */
std::string HttpClient::del() { return httpCommon(HTTP_DELETE); }

/**
 * Http PUT method.
 */
std::string HttpClient::put() { return httpCommon(HTTP_PUT); }
