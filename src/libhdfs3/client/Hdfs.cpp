...
struct HdfsFileInternalWrapper {
 public:
  HdfsFileInternalWrapper() : input(true), stream(NULL) {}
  ...
  void setStream(void *stream) { this->stream = stream; }
...
struct HdfsFileSystemInternalWrapper {
 public:
  HdfsFileSystemInternalWrapper(FileSystem *fs) : filesystem(fs) {}

  ~HdfsFileSystemInternalWrapper() { delete filesystem; }
...
class DefaultConfig {
 public:
  ...
  shared_ptr<Config> getConfig() { return conf; }
...
struct hdfsBuilder {
 public:
  hdfsBuilder() : conf(DefaultConfig().getConfig()), port(0) {}
...
static void handleException(Hdfs::exception_ptr error) {
  try {
    ...
    std::string buffer;
    LOG(Hdfs::Internal::LOG_ERROR, "Unexpected exception %s: %s",
        typeid(e).name(), Hdfs::Internal::GetExceptionDetail(e, buffer));
    errno = EINTERNAL;
...
const char *hdfsGetLastError() { return ErrorMessage; }
...
hdfsFS hdfsBuilderConnect(struct hdfsBuilder *bld) {
  ...
  try {
      ...
      if (bld->port == 0 && uriobj->port == 0) {
          ...
          ss << uriobj->server << ":" << (uriobj->port ? uriobj->port :
  bld->port);
      }
...
void hdfsFreeBuilder(struct hdfsBuilder *bld) { delete bld; }

void hdfsBuilderSetForceNewInstance(struct hdfsBuilder *bld) { assert(bld); }
...
void hdfsConfStrFree(char *val) { delete[] val; }
...
hdfsFile hdfsOpenFile(hdfsFS fs, const char *path, int flags, int bufferSize,
  ...
  PARAMETER_ASSERT(bufferSize >= 0 && replication >= 0 && blocksize >= 0, NULL,
                   EINVAL);
  PARAMETER_ASSERT(
      !(flags & O_RDWR) && !((flags & O_EXCL) && (flags & O_CREAT)), NULL,
      ENOTSUP);
  ...
  try {
    ...
    if ((flags & O_CREAT) || (flags & O_APPEND) || (flags & O_WRONLY)) {
      ...
      file->setInput(false);
      os = new OutputStream;
      os->open(fs->getFilesystem(), path, internalFlags, 0777, false,
...
tSize hdfsPread(hdfsFS fs, hdfsFile file, tOffset pos, void *buffer,
                tSize length) {
...
tSize hdfsWrite(hdfsFS fs, hdfsFile file, const void *buffer, tSize length) {
  ...
  try {
    file->getOutputStream().append(static_cast<const char *>(buffer), length);
    return length;
  } catch (const std::bad_alloc &e) {
    SetErrorMessage("Out of memory");
    errno = ENOMEM;
...
int hdfsSync(hdfsFS fs, hdfsFile file) { return hdfsHSync(fs, file); }
...
int hdfsSetReplication(hdfsFS fs, const char *path, int16_t replication) {
  PARAMETER_ASSERT(fs && path && strlen(path) > 0 && replication > 0, -1,
                   EINVAL);
...
static void ConstructHdfsEncryptionZoneInfo(
    hdfsEncryptionZoneInfo *infoEn,
...
static void ConstructHdfsEncryptionFileInfo(
    hdfsEncryptionFileInfo *infoEn, Hdfs::FileEncryptionInfo *enStatus) {
  ...
  infoEn->mEzKeyVersionName =
      const_cast<char *>(enStatus->getEzKeyVersionName().c_str());
}
...
static void ConstructHdfsFileInfo(hdfsFileInfo *infos,
  ...
  for (size_t i = 0; i < size; ++i) {
    ...
    infos[i].mKind =
        status[i].isDirectory() ? kObjectKindDirectory : kObjectKindFile;
    ...
    if (status[i].isFileEncrypted()) {
      ...
      memset(infos[i].mHdfsEncryptionFileInfo, 0,
             sizeof(hdfsEncryptionFileInfo));
      ConstructHdfsEncryptionFileInfo(infos[i].mHdfsEncryptionFileInfo,
                                      status[i].getFileEncryption());
    }
  }
}
...
int hdfsChown(hdfsFS fs, const char *path, const char *owner,
              const char *group) {
  PARAMETER_ASSERT(fs && path && strlen(path) > 0, -1, EINVAL);
  PARAMETER_ASSERT((owner && strlen(owner) > 0) || (group && strlen(group) > 0),
                   -1, EINVAL);
...
int hdfsTruncate(hdfsFS fs, const char *path, tOffset pos, int *shouldWait) {
  PARAMETER_ASSERT(fs && path && strlen(path) > 0 && pos >= 0 && shouldWait, -1,
                   EINVAL);
...
static Namenode *hdfsGetConfiguredNamenodesInternal(const char *nameservice,
  ...
  std::vector<NamenodeInfo> namenodeInfos =
      NamenodeInfo::GetHANamenodeInfo(nameservice, *conf);
...
Namenode *hdfsGetHANamenodesWithConfig(const char *conf,
  ...
  PARAMETER_ASSERT(conf && strlen(conf) > 0 && nameservice && size, NULL,
                   EINVAL);
...
static void ConstructFileBlockLocation(Hdfs::BlockLocation &bl,
...
BlockLocation *hdfsGetFileBlockLocations(hdfsFS fs, const char *path,
  ...
  try {
    std::vector<Hdfs::BlockLocation> locations =
        fs->getFilesystem().getFileBlockLocations(path, start, length);
    size = locations.size();
...
int hdfsCreateEncryptionZone(hdfsFS fs, const char *path, const char *keyName) {
  PARAMETER_ASSERT(
      fs && path && strlen(path) > 0 && keyName && strlen(keyName) > 0, -1,
      EINVAL);
...
hdfsEncryptionZoneInfo *hdfsGetEZForPath(hdfsFS fs, const char *path) {
  ...
  try {
    ...
    /* If out of memory error occurred, free hdfsEncryptionZoneInfo array's
     * memory. */
    errno = ENOMEM;
  } catch (...) {
    SetLastException(Hdfs::current_exception());
