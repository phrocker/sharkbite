...
OutputStreamImpl::OutputStreamImpl()
    : /*heartBeatStop(true),*/ closed(true),
      isAppend(false),
      syncBlock(false),
      checksumSize(0),
      chunkSize(0),
...
void OutputStreamImpl::open(shared_ptr<FileSystemInter> fs, const char *path,
  ...
  if (!(flag == Create || flag == (Create | SyncBlock) || flag == Overwrite ||
        ...
        flag == (Create | Overwrite | SyncBlock) || flag == (Create | Append) ||
...
void OutputStreamImpl::openInternal(shared_ptr<FileSystemInter> fs,
  ...
  if (0 != this->blockSize % chunkSize) {
    THROW(InvalidParameter,
          "OutputStreamImpl: block size %" PRId64
          " is not the multiply of chunk size %d.",
...
#ifdef USE_KRB5
        if (cryptoCodec == NULL) {
          auth = shared_ptr<RpcAuth>(new RpcAuth(
              fs->getUserInfo(), RpcAuth::ParseMethod(conf->getKmsMethod())));
          kcp =
              shared_ptr<KmsClientProvider>(new KmsClientProvider(auth, conf));
...
void OutputStreamImpl::append(const char *buf, int64_t size) {
  LOG(DEBUG3,
      "append file %s size is %" PRId64 ", offset %" PRId64
      " next pos %" PRId64,
      path.c_str(), size, cursor, size + cursor);
...
#else
  pipeline = shared_ptr<Pipeline>(new PipelineImpl(
  ...
  while (true) {
    ...
    if (closeTimeout > 0) {
      ...
      if (ToMilliSeconds(start, end) >= closeTimeout) {
        if (throwError) {
          THROW(HdfsIOException,
                "OutputStreamImpl: timeout when complete file %s, timeout "
                "interval %d ms.",
        ...
        try {
            ...
            try {
                try {
                    ...
                    NESTED_THROW(Hdfs::HdfsIOException, "Failed to send heart
beat, path: %s", path.c_str());
                }
            } catch (...) {
             ...
             * keep quiet if we run out of memory, since writing log need
memory,
