...
PipelineImpl::PipelineImpl(bool append, const char* path,
                           const SessionConfig& conf,
                           shared_ptr<FileSystemInter> filesystem,
                           int checksumType, int chunkSize, int replication,
                           int64_t bytesSent, PacketPool& packetPool,
  ...
  if (append) {
    LOG(DEBUG2,
        "create pipeline for file %s to append to %s at position %" PRId64,
...
int PipelineImpl::findNewDatanode(const std::vector<DatanodeInfo>& original) {
  if (nodes.size() != original.size() + 1) {
    THROW(HdfsIOException,
          "Failed to acquire a datanode for block %s from namenode.",
...
void PipelineImpl::transfer(const ExtendedBlock& blk, const DatanodeInfo& src,
  ...
  if (!resp.ParseFromArray(&buf[0], size)) {
    THROW(HdfsIOException,
          "cannot parse datanode response from %s fro block %s.",
          src.formatAddress().c_str(), lastBlock->toString().c_str());
  }

  if (Status::DT_PROTO_SUCCESS != resp.status()) {
    THROW(HdfsIOException,
          "Failed to transfer block to a new datanode %s for block %s.",
          targets[0].formatAddress().c_str(), lastBlock->toString().c_str());
  }
}

bool PipelineImpl::addDatanodeToPipeline(
...
void PipelineImpl::checkPipelineWithReplicas() {
  if (static_cast<int>(nodes.size()) < replication) {
    ...
    LOG(WARNING,
        "the number of nodes in pipeline is %d [%s], is less than the expected "
        "number of replica %d for block %s file %s",
...
void PipelineImpl::buildForAppendOrRecovery(bool recovery) {
  ...
  do {
    ...
    if (errorIndex >= 0) {
      assert(lastBlock);
      LOG(LOG_ERROR,
          "Pipeline: node %s is invalid and removed from pipeline when %s "
          "block %s for file %s, stage = %s.",
    ...
    try {
      ...
      /*
       * Check if the number of datanodes in pipeline satisfy the replication
       * requirement, add new datanode if not
       */
      if (stage != PIPELINE_SETUP_CREATE && stage != PIPELINE_CLOSE &&
        ...
        if (!addDatanodeToPipeline(excludedNodes)) {
          THROW(
              HdfsIOException,
              "Failed to add new datanode into pipeline for block: %s file %s, "
              "set \"output.replace-datanode-on-failure\" to \"false\" to "
      ...
      lastException = current_exception();
      /*
       * Set recovery flag to true in case of failed to create a pipeline for
       * appending.
       */
      recovery = true;
      LOG(LOG_ERROR,
     ...
     * retry count in case infinite loop. it may caused by rpc call throw
     * HdfsIOException
     */
    if (errorIndex < 0) {
      --retry;
...
void PipelineImpl::locateNextBlock(
  ...
  while (true) {
    try {
      lastBlock = filesystem->addBlock(path, lastBlock.get(), excludedNodes);
      assert(lastBlock);
      return;
    } catch (const NotReplicatedYetException& e) {
      LOG(DEBUG1,
          "Got NotReplicatedYetException when try to addBlock for block %s, "
...
void PipelineImpl::buildForNewBlock() {
  ...
  do {
    ...
    try {
      ...
      LOG(INFO,
          "Retry to allocate a new empty block for file %s, last block %s, "
          "excluded nodes %s.",
          path.c_str(), lastBlockName,
          FormatExcludedNodes(excludedNodes).c_str());
      ++retryAllocNewBlock;
    ...
    try {
      ...
      LOG(LOG_ERROR, "Failed to abandon useless block %s for file %s.\n%s",
          lastBlock->toString().c_str(), path.c_str(),
          GetExceptionDetail(e, buffer));
      throw;
    }
    ...
    if (errorIndex >= 0) {
      ...
      /*
       * we don't known what happened, no datanode is reported failure, reduce
       * retry count in case of infinite loop.
       */
      --retry;
    }
  } while (retry);
...
void PipelineImpl::checkBadLinkFormat(const std::string& n) {
  ...
  LOG(FATAL,
      "Cannot parser the firstBadLink string %s, it should be a bug or "
...
void PipelineImpl::createBlockOutputStream(const Token& token, int64_t gs,
  ...
  try {
    ...
    reader =
        shared_ptr<BufferedSocketReader>(new BufferedSocketReaderImpl(*sock));
    ...
    if (!resp.ParseFromArray(&buf[0], size)) {
      THROW(HdfsIOException,
            "cannot parse datanode response from %s for block %s.",
    ...
    if (Status::DT_PROTO_SUCCESS != pipelineStatus) {
      ...
      if (Status::DT_PROTO_ERROR_ACCESS_TOKEN == pipelineStatus) {
        ...
        THROW(HdfsIOException,
              "Bad connect ack with firstBadLink as %s for block %s",
              firstBadLink.c_str(), lastBlock->toString().c_str());
      }
    }
  ...
  try {
    ...
    NESTED_THROW(HdfsIOException,
                 ...
                 lastBlock->toString().c_str(), (recovery ? "true" : "false"),
                 gs);
  }
}
  ...
  do {
    try {
      if (failover) {
        ...
        assert(sock);
        sock->writeFully(buffer.getBuffer(), buffer.getSize(), writeTimeout);
...
void PipelineImpl::checkResponse(bool wait) {
  ...
  if (readable) {
    ...
    THROW(HdfsIOException,
          "Timeout when reading response for block %s, datanode %s do not "
          "response.",
          lastBlock->toString().c_str(), nodes[0].formatAddress().c_str());
  }
}

void PipelineImpl::flush() { waitForAcks(true); }
