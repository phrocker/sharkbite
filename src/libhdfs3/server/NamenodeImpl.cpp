...
NamenodeImpl::NamenodeImpl(std::string host, std::string port,
                           const std::string& tokenService,
                           const SessionConfig& c, const RpcAuth& a)
    : auth(a),
      client(RpcClient::getClient()),
      conf(c),
      protocol(NAMENODE_VERSION, NAMENODE_PROTOCOL, DELEGATION_TOKEN_KIND),
...
// Idempotent
void NamenodeImpl::getBlockLocations(
...
FileNotFoundException, UnresolvedLinkException, HdfsIOException) */
{
  try {
    ...
    UnWrapper<FileNotFoundException, UnresolvedLinkException, HdfsIOException>
        unwrapper(e);
...
void NamenodeImpl::create(const std::string& src, const Permission& masked,
                          ...
                          bool createParent, short replication,
...
UnresolvedLinkException, HdfsIOException) */
{
  try {
    ...
    request.set_src(src);
    request.add_cryptoprotocolversion(
    ...
    UnWrapper<AlreadyBeingCreatedException, DSQuotaExceededException,
              FileAlreadyExistsException, FileNotFoundException,
              NSQuotaExceededException, ParentNotDirectoryException,
              UnresolvedLinkException, HdfsIOException>
        unwrapper(e);
 ...
 HdfsIOException) */
{
  try {
    ...
    UnWrapper<DSQuotaExceededException, FileNotFoundException,
              UnresolvedLinkException, HdfsIOException>
        unwrapper(e);
    unwrapper.unwrap(__FILE__, __LINE__);
  }
...
// Idempotent
void NamenodeImpl::setOwner(const std::string& src, const std::string& username,
...
shared_ptr<LocatedBlock> NamenodeImpl::addBlock(
 ...
 UnresolvedLinkException, HdfsIOException) */
{
  try {
    ...
    UnWrapper<FileNotFoundException, NotReplicatedYetException,
              UnresolvedLinkException, HdfsIOException>
        unwrapper(e);
    unwrapper.unwrap(__FILE__, __LINE__);
  }
...
(UnresolvedLinkException, HdfsIOException) { try { ConcatRequestProto request;
...
void NamenodeImpl::getLease(const std::string& src,
        ...
        (HdfsIOException, UnresolvedLinkException) */
{
  try {
    ...
    UnWrapper<InvalidParameter, AlreadyBeingCreatedException,
              FileNotFoundException, UnresolvedLinkException, HdfsIOException>
        unwrapper(e);
    unwrapper.unwrap(__FILE__, __LINE__);
  }
          ...
          UnresolvedLinkException, HdfsIOException) */
{
  try {
    ...
    UnWrapper<FileAlreadyExistsException, FileNotFoundException,
              NSQuotaExceededException, ParentNotDirectoryException,
              UnresolvedLinkException, HdfsIOException>
        unwrapper(e);
    unwrapper.unwrap(__FILE__, __LINE__);
...
// Idempotent
bool NamenodeImpl::getListing(
    const std::string& src, const std::string& startAfter, bool needLocation,
...
/*bool NamenodeImpl::recoverLease(const std::string & src,
                                const std::string & clientName) {   throw
(HdfsIOException) try { RecoverLeaseRequestProto request;
...
HdfsIOException) { try { SetQuotaRequestProto request; SetQuotaResponseProto
response; request.set_path(path); request.set_namespacequota(namespaceQuota);
...
dirPerm, bool createParent)  throw (AccessControlException,
         FileAlreadyExistsException, FileNotFoundException,
...
void NamenodeImpl::updatePipeline(
    const std::string& clientName, const ExtendedBlock& oldBlock,
    const ExtendedBlock& newBlock, const std::vector<DatanodeInfo>& newNodes,
...
bool NamenodeImpl::createEncryptionZone(const std::string& src,
...
EncryptionZoneInfo NamenodeImpl::getEncryptionZoneInfo(const std::string& src,
                                                       bool* exist)
/* throw (FileNotFoundException,
 UnresolvedLinkException, HdfsIOException) */
{
...
// Idempotent
bool NamenodeImpl::listEncryptionZones(const int64_t id,
                                       std::vector<EncryptionZoneInfo>& ezl)
/* throw (AccessControlException,FileNotFoundException, UnresolvedLinkException,
   HdfsIOException) */
{
  try {
