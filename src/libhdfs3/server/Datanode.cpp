...
DatanodeImpl::DatanodeImpl(const std::string& host, uint32_t port,
    ...
    : auth(a),
      ...
      conf(c),
      protocol(DATANODE_VERSION, DATANODE_PROTOCOL, BLOCK_TOKEN_KIND),
      server(host, port) {
...
void DatanodeImpl::invoke(const RpcCall& call, bool reuse) {
  ...
  try {
    ...
    assert(
        false &&
        "HdfsFailoverException should be always a wrapper of other exception");
  } catch (...) {
    channel.close(true);
    throw;
  }
...
int64_t DatanodeImpl::getReplicaVisibleLength(const ExtendedBlock& b) {
  try {
    ...
    invoke(RpcCall(true, "getReplicaVisibleLength", &request, &response),
           false);
