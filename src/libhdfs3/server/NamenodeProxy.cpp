  ...
  for (size_t i = 0; i < namenodeInfos.size(); ++i) {
    ...
    if (nninfo.size() != 2) {
      THROW(InvalidParameter,
            "Cannot create namenode proxy, %s does not contain host or port",
            namenodeInfos[i].getRpcAddr().c_str());
    }

    namenodes.push_back(shared_ptr<Namenode>(
        new NamenodeImpl(nninfo[0], nninfo[1], clusterid, c, a)));
  }

  if (enableNamenodeHA && !namenodeInfos.empty()) {
...
void NamenodeProxy::create(const std::string& src, const Permission& masked,
                           ...
                           int64_t blockSize) {
  NAMENODE_HA_RETRY_BEGIN();
  namenode->create(src, masked, clientName, flag, createParent, replication,
                   blockSize);
  NAMENODE_HA_RETRY_END();
}
...
shared_ptr<LocatedBlock> NamenodeProxy::addBlock(
...
shared_ptr<LocatedBlock> NamenodeProxy::getAdditionalDatanode(
  ...
  NAMENODE_HA_RETRY_BEGIN();
  return namenode->getAdditionalDatanode(src, blk, existings, storageIDs,
                                         excludes, numAdditionalNodes,
                                         clientName);
  NAMENODE_HA_RETRY_END();
  assert(!"should not reach here");
  return shared_ptr<LocatedBlock>();
...
{ NAMENODE_HA_RETRY_BEGIN(); namenode->reportBadBlocks(blocks);
    NAMENODE_HA_RETRY_END();
}*/
...
dirPerm, bool createParent) { NAMENODE_HA_RETRY_BEGIN();
...
bool NamenodeProxy::createEncryptionZone(const std::string& src,
...
EncryptionZoneInfo NamenodeProxy::getEncryptionZoneInfo(const std::string& src,
                                                        bool* exist) {
  NAMENODE_HA_RETRY_BEGIN();
...
bool NamenodeProxy::listEncryptionZones(const int64_t id,
