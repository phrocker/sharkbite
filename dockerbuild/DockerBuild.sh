...
function usage {
  echo "Usage: ./DockerBuild.sh -v <SHARKBITE_VERSION> [additional options]"
  echo "Options:"
  echo "-v, --SHARKBITE-version  SHARKBITE version number to be used (required)"
  ...
  echo "-o  --options         SHARKBITE options string"
...
function dump_ccache() {
  ...
  docker cp "${container_id}:/opt/SHARKBITE/.ccache/." "${docker_ccache_dump_location}"
...
while [[ $# -gt 0 ]]; do
  ...
  -c | --cmake-param)
    ...
    ;;
  -o | --options)
    BUILD_ARGS+=("--build-arg" "SHARKBITE_OPTIONS=$2")
    shift
    shift
    ;;
  -h | --help)
...
echo "SHARKBITE Version: ${SHARKBITE_VERSION}"
...
BUILD_ARGS+=("--build-arg" "UID=${UID_ARG}"
             ...
             "--build-arg" "SHARKBITE_VERSION=${SHARKBITE_VERSION}"
...
if [ -n "${DISTRO_NAME}" ]; then
  ...
  echo docker build build "${BUILD_ARGS[@]}" -f "${DOCKERFILE}" ${PLATFORMS} ${PUSH_OR_LOAD} ${TAGGING_CMD} ..
  # shellcheck disable=SC2086
  ...
  if [ -n "${DOCKER_CCACHE_DUMP_LOCATION}" ]; then
    docker buildx build "${BUILD_ARGS[@]}" -f "${DOCKERFILE}" --target build -t SHARKBITE_build ..
    dump_ccache "SHARKBITE_build" "${DOCKER_CCACHE_DUMP_LOCATION}"
  fi
  # shellcheck disable=SC2086
...
if [ -n "${DUMP_LOCATION}" ]; then
  ...
  docker cp "${container_id}:/opt/SHARKBITE/build/nifi-SHARKBITE-cpp-${SHARKBITE_VERSION}.tar.gz" "${DUMP_LOCATION}/nifi-SHARKBITE-cpp-${SHARKBITE_VERSION}-${TARGZ_TAG}.tar.gz"
  docker rm -f "${container_id}"
fi
